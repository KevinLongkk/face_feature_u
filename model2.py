...
def model(regularzier):
    ...
    with tf.variable_scope('conv1'):
        conv1_weights = tf.get_variable('weight', shape=[11, 11, 3, 96], initializer=tf.truncated_normal_initializer(0.1))
        conv1_biases = tf.get_variable('biase', shape=[96], initializer=tf.constant_initializer(0.1))
        conv1 = tf.nn.conv2d(image, conv1_weights, strides=[1, 4, 4, 1], padding='VALID')
    ...
    with tf.variable_scope('conv2'):
        conv2_weights = tf.get_variable('weight', shape=[5, 5, 96, 256], initializer=tf.truncated_normal_initializer(0.1))
        conv2_biases = tf.get_variable('biase', shape=[256], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('conv3'):
        conv3_weights = tf.get_variable('weight', shape=[3, 3, 256, 384], initializer=tf.truncated_normal_initializer(0.1))
        conv3_biases = tf.get_variable('biase', shape=[384], initializer=tf.constant_initializer(0.1))
        conv3 = tf.nn.conv2d(pool2, conv3_weights, strides=[1, 1, 1, 1], padding='SAME')
        relu3 = tf.nn.relu(tf.nn.bias_add(conv3, conv3_biases))
        pool3 = tf.nn.max_pool(relu3, ksize=[1, 3, 3, 1], strides=[1, 1, 1, 1], padding='SAME')

    with tf.variable_scope('conv4'):
        conv4_weights = tf.get_variable('weight', shape=[3, 3, 384, 384], initializer=tf.truncated_normal_initializer(0.1))
        conv4_biases = tf.get_variable('biase', shape=[384], initializer=tf.constant_initializer(0.1))
        conv4 = tf.nn.conv2d(pool3, conv4_weights, strides=[1, 1, 1, 1], padding='SAME')
        relu4 = tf.nn.relu(tf.nn.bias_add(conv4, conv4_biases))
        pool4 = tf.nn.max_pool(relu4, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')

    with tf.variable_scope('conv5'):
        conv5_weights = tf.get_variable('weight', shape=[3, 3, 384, 256], initializer=tf.truncated_normal_initializer(0.1))
        conv5_biases = tf.get_variable('biase', shape=[256], initializer=tf.constant_initializer(0.1))
